...
- **Print / PDF export**: opens the browser's print dialog ("Save as PDF" is
  ...
  dialog); with `options.pdfChrome: false` the module drops its own banner
  and logo footer, leaving just the grid for the host to wrap in its own
  header/footer.
...
| `locale`           | `"fr" \| "en" \| "de" \| "es" \| "it" \| "zh"`                                                                         | Language of every built-in UI string — toolbar, "Settings" panel, context menus, student / colour pickers, "add a label" dialog, print-sheet blanks (default `"fr"`). Region tags are accepted (`"en-GB"` → `en`); an unknown value falls back to French. `buildPrintSheet(state, { locale })` takes the same for a host driving its own PDF.                    |
...
| `onPrint`          | `(state, {teacher, logoUrl, showLevel, nameDisplay, nameFit, levelFit, printOrientation, printPaper, chrome}) => void` | Optional override for `layout.print()` — if supplied, called instead of opening the browser's print dialog (e.g. to render a PDF from `buildPrintSheet()` and show it however the host app displays PDFs). The print settings are passed straight through so the host can apply them in its own PDF renderer; `chrome` reflects `options.pdfChrome`.             |
| `logoUrl`          | `string`                                                                                                               | Optional host-app logo URL, shown centered at the bottom of the print/PDF sheet (mirrors the site's other PDF exports). Suppressed when `pdfChrome` is `false`.                                                                                                                                                                                                  |
| `pdfChrome`        | `boolean`                                                                                                              | Whether `buildPrintSheet()` / `print()` add the module's own page chrome — the school/teacher/meta banner and the logo footer (default `true`). Set `false` when the host wraps the captured grid in its own header/footer (e.g. a server-side PDF pipeline); the grid, plus the `printOrientation`/`printPaper` datasets, is then all the sheet carries.        |
...
`buildPrintSheet(state, {teacher, editableTeacherInputs, logoUrl, showLevel, nameDisplay, nameFit, levelFit, printOrientation, printPaper, chrome})`,
...
`sheet.dataset` for a host driving its own PDF renderer; `chrome: false`
drops the banner and logo footer so the sheet is the bare grid — fully
